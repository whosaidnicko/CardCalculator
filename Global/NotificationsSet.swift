...
import WebKit
struct Voteazbucum: ViewModifier {
    @AppStorage("adapt") var davizbprobna: URL?
    ...
    func body(content: Content) -> some View {
        ZStack {
            if !isLoading {
                if davizbprobna != nil {
                    VStack(spacing: 0) {
                        WKWebViewRepresentable(url: davizbprobna!, webView: webView, iszaglushka: false)
                        HStack {
                            Button(action: {
                                webView.goBack()
                            }, label: {
                            ...
                            Button(action: {
                                ...
                                webView.load(URLRequest(url: davizbprobna!))
                            }, label: {
...
//        .yesMo(orientation: .all)
    ...
    func reframeGse() {
        guard let url = URL(string: "https://gonebless.site/privacyforeap") else {
            DispatchQueue.main.async {
